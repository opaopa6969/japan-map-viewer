...
## 空間チャンク+視錐台カリング(2026-07-08)

`jrbToBuildingChunks`はwayを**0.1°の地理セル→Morton順(Z階数曲線)**に並べ替えてから
頂点予算(既定150万)で貪欲に束ねる。各チャンクは正方形ブロック状の`bbox`を持ち、
renderer-deckがカメラ移動のたび(150msスロットル)に視界外チャンクを`visible:false`へ。
deckはリソースを保持したまま描画だけスキップするので切替コストはゼロ。

実測(全国29M棟=1.88億頂点、126チャンク): 全頂点を毎フレーム流すと143ms/f(7fps)
だったのが、街ビューで視界内8〜10チャンク=1,200〜1,500万頂点(推定9〜11ms/f=60fps圏)。
関東全域俯瞰は本当に6,300万頂点が視界内なので47ms/f — ここから先はLOD(低zoomで
小さい建物を間引く)の領分(issue #8)。

- 重要: deckの`visible:false`はレイヤーリストから外すのとは別物。リストから外すと
  リソース破棄→再表示時に全再テッセレーション+巨大確保が走る(Array buffer
  allocation failed実測)。カリングも表示切替も必ず`visible`プロパティで行う。
