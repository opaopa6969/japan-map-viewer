...
type別 data: `network`(nodes+edges のグラフ) / `paths`(実世界のポリライン=鉄道路線・道路等。
ノード共有なしの線の束) / `extrusion`(points+value → 柱。2Dは円にフォールバック) /
`movers`(tokens+route[{lat,lon,t}] → clock で線形補間して毎フレーム移動。`loop: true` で周回) /
`markers`(絵文字アイコン+ラベルの静止点)。

## 鉄道データ(国土数値情報 N02)と メタデータAPI

`npm run fetch:railways` が N02(鉄道)を取得し、路線名・運営会社・事業者種別・駅名・
駅コード/グループコード付きで `public/data/japan-railways.json`(約10MB、コミット済み)に
圧縮する。`/layers-demo` の「実路線」「東海道新幹線を実線路で走らせる」がこれを使う
(movers の route を実線路ポリラインに沿わせる)。

デモサーバはメタデータ検索APIも提供する(全量10MBをブラウザに送らない):

- `GET /api/railways` — サマリ(件数・事業者種別ラベル)
- `GET /api/railways/lines?q=山手&company=&kind=` — 路線メタデータ(名寄せ済み・ジオメトリなし)
- `GET /api/railways/line?name=山手線&company=東日本旅客鉄道` — 1路線のジオメトリ(区間)+駅
- `GET /api/railways/stations?q=渋谷&limit=50` — 駅名の部分一致検索

## 高速道路(N06)と OSM道路

- `npm run fetch:highways` — 国土数値情報 高速道路時系列(N06)。路線名・**供用開始年**付きで
  `public/data/japan-highways.json`(約3MB、コミット済み)。供用年があるので
  「年を進めると道路網が伸びる」タイムライン再生にも使える。
- **`npm run fetch:snapshot`** — 生成済みJRB(全国道路+全国建物2,904万棟)を
  [GitHub Release(data-v1)](https://github.com/opaopa6969/japan-map-viewer/releases/tag/data-v1)から
  取得(ABR-Snapshot方式)。**再生成20分を待たずにフル機能で動く**。`--assets kanto|all`で追加分
- `npm run fetch:osm-roads -- --region shikoku|kanto|…|japan` — OpenStreetMap(Geofabrik)から
  道路網を抽出。一般道は全国JSONにすると数百MBになるため、**自作バイナリcodec
  "JRB1"**(`lib/road-codec.mjs`: 座標1e5量子化+delta+zigzag varint+道路名テーブル)で
  `data/osm-roads-<region>.jrb` に圧縮する(四国: 88MB PBF → 1.8MB、gitignore対象)。
  PBFの解析も純JS(`lib/osm-pbf.mjs`、依存ゼロ)。既定クラスは幹線系
  (motorway〜tertiary+link)、`--classes` で変更可。
- サーバは `.jrb` を**初回アクセスで全部オンメモリに開き**(グリッド索引を構築)、
  ビューポート分だけ部分デコードして返す(ミニタイルサーバ相当):
  - `GET /api/roads/meta` — 読み込んだ地域・クラス・件数
  - `GET /api/roads?bbox=lon0,lat0,lon1,lat1&classes=primary,…&limit=4000` —
    bbox交差のwayをGeoJSON風JSONで(上限超過は `truncated: true` を明示)
- `/layers-demo` の「実道路(OSM)」トグルは moveend ごとにこのAPIを叩き、
  ズームに応じたクラスを `updateLayerData` で差し替える。

## 建物データ(PLATEAU / OSM)

- `npm run fetch:plateau -- --code 13101` — Project PLATEAU(国交省)の**3D Tiles建築物モデル**
  (実測高さ・LOD1/2)を区市町村コード指定でDLし `public/data/plateau/<code>/`(gitignore)に展開。
  tileset.json + b3dm 形式なので deck.gl の `Tile3DLayer`(=レイヤー種別 `tiles3d`)が
  そのまま読める。既定は東京23区2020年度・テクスチャ無し版(千代田区で約40MB/176タイル)。
- `npm run fetch:osm-buildings -- --region kanto` — OSM建物フットプリント+高さ
  (heightタグ→building:levels×3m)をJRB v2(withValues=高さdm)に圧縮
  (関東: 611万棟→170MB。ストリーミングエンコーダでOOM回避)。
  サーバが `/api/buildings?bbox=` でビューポート配信し、レイヤー種別 `polygons`
  (deck=SolidPolygonLayer押し出し/2D=フットプリント塗り/3D=ExtrudeGeometry)で描く。
- `/layers-demo` の「実建物(OSM)」(zoom14+ビューポート連動)と「PLATEAU 3D Tiles」トグルが実例。
- 補足: `tiles3d` はGL(deck)レンダラ専用(`supportsLayerType()` が false を返す)。
