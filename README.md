...
- `npm run fetch:plateau -- --code 13101` — Project PLATEAU(国交省)の**3D Tiles建築物モデル**
  ...
  tileset.json + b3dm 形式なので deck.gl の `Tile3DLayer` がそのまま読める。
  既定は東京23区2020年度・テクスチャ無し版(千代田区で約40MB/176タイル)。
- OSM建物は `fetch:osm-roads` が落とす同じ地域PBFに `building=*` として入っている
  (lib/osm-pbf.mjs でそのまま読める)。フットプリント+高さ/階数タグ→押し出しの
  変換パイプラインは今後(全国だと数百万ポリゴンになるため地域単位を想定)。
